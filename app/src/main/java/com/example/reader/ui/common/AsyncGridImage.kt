...
private val PLACEHOLDER_DRAWABLE = ColorDrawable(Color(0xFF1C1C1E).hashCode())
...
/**
 * 网格/列表缩略图（支持视频缓存）。
 * - 优先使用 [MediaItem.thumbnailPath]（L2 磁盘缓存直读，硬件加速）
 * - 图片项：直接加载，.size(300) 避免解码全分辨率
 * - 视频项回退：Coil VideoFrameDecoder 实时抽帧 → 成功后自动回写 L2 缓存
 * - 所有请求统一应用 crossfade(300) + 深色占位符消除 UI pop-in
 */
@Composable
fun AsyncGridImage(
    item: MediaItem,
